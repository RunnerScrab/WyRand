...
impl WyRand {
    ...
    pub fn fill_std_normal_f32(&mut self, buf: &mut [f32]) {
        ...
        for chunk in iter.by_ref() {
            ...
            let r = fptricks::batch_approx_sqrt_f32(fptricks::batch_fmadd_f32(
                fptricks::batch_approx_ln_f32(u1),
                -2.0,
                0.0,
            ));
            let (s, c) = fptricks::batch_approx_sin_cos_f32(fptricks::batch_fmadd_f32(
                u2,
                Self::TWO_PI_F32,
                0.0,
            ));
        ...
        if i < rem.len() {
            rem[i] = self.next_std_normal_f32();
        }
    }
    ...
    pub fn fill_std_normal_f64(&mut self, buf: &mut [f64]) {
        ...
        for chunk in iter.by_ref() {
            ...
            let r = fptricks::batch_approx_sqrt_f64(fptricks::batch_fmadd_f64(
                fptricks::batch_approx_ln_f64(u1),
                -2.0,
                0.0,
            ));
            let (s, c) = fptricks::batch_approx_sin_cos_f64(fptricks::batch_fmadd_f64(
                u2,
                Self::TWO_PI_F64,
                0.0,
            ));
        ...
        if i < rem.len() {
            rem[i] = self.next_std_normal_f64();
        }
    }
        ...
        S: ParamSource<f32>,
    {
        ...
        for (i, chunk) in iter.by_ref().enumerate() {
            ...
            let res1 =
                fptricks::batch_add_cols_f32(&m1, &fptricks::batch_mul_3_cols_f32(&r, &s, &s1));
            let res2 =
                fptricks::batch_add_cols_f32(&m2, &fptricks::batch_mul_3_cols_f32(&r, &c, &s2));
        ...
        if i < rem.len() {
            rem[i] = self
                .next_std_normal_f32()
                .mul_add(sigma.get(offset + i), mode.get(offset + i));
        }
    }
        ...
        S: ParamSource<f64>,
    {
        ...
        for (i, chunk) in iter.by_ref().enumerate() {
            ...
            let res1 =
                fptricks::batch_add_cols_f64(&m1, &fptricks::batch_mul_3_cols_f64(&r, &s, &s1));
            let res2 =
                fptricks::batch_add_cols_f64(&m2, &fptricks::batch_mul_3_cols_f64(&r, &c, &s2));
    ...
    #[inline(always)]
    pub fn fill_normal_w_clamped_err_f32<M, S, L>(
        &mut self,
        buf: &mut [f32],
        mode: M,
        sigma: S,
        ...
        L: ParamSource<f32>,
    {
        ...
        for (i, chunk) in iter.by_ref().enumerate() {
            ...
            chunk.copy_from_slice(&fptricks::batch_fmadd_cols_f32(
                arr,
                sigma.chunk::<8>(offset),
                mode.chunk::<8>(offset),
            ));
        }
        let rem = iter.into_remainder();
    ...
    #[inline(always)]
    pub fn fill_normal_w_clamped_err_f64<M, S, L>(
        &mut self,
        buf: &mut [f64],
        mode: M,
        sigma: S,
        ...
        L: ParamSource<f64>,
    {
        ...
        for (i, chunk) in iter.by_ref().enumerate() {
            ...
            chunk.copy_from_slice(&fptricks::batch_fmadd_cols_f64(
                arr,
                sigma.chunk::<4>(offset),
                mode.chunk::<4>(offset),
            ));
        }
        let rem = iter.into_remainder();
    ...
    #[inline(always)]
    pub fn fill_split_normal_f32<M, SLO, SHI>(
        &mut self,
        buf: &mut [f32],
        mode: M,
        sigma_lo: SLO,
        ...
        SHI: ParamSource<f32>,
    {
        ...
        for (i, chunk) in iter.by_ref().enumerate() {
            ...
            for j in 0..8 {
                ...
                chunk[j << 1] = f32::from_bits(
                    (xltz & (m + x * sl1[j]).to_bits()) | (!xltz & (m + x * sh1[j]).to_bits()),
                );
                let x2 = r[j] * c[j];
                let m2v = m2[j];
                let x2ltz: u32 = ((x2 < 0.0) as u32).wrapping_neg();
                chunk[(j << 1) + 1] = f32::from_bits(
                    (x2ltz & (m2v + x2 * sl2[j]).to_bits())
        ...
        for (i, val) in rem.iter_mut().enumerate() {
            ...
            let xltz: u32 = ((x < 0.0) as u32).wrapping_neg();
            *val = f32::from_bits(
                (xltz & (m + x * s_lo).to_bits()) | (!xltz & (m + x * s_hi).to_bits()),
            );
        }
    }

    #[inline(always)]
    pub fn fill_split_normal_f64<M, SLO, SHI>(
        &mut self,
        buf: &mut [f64],
        mode: M,
        sigma_lo: SLO,
        ...
        SHI: ParamSource<f64>,
    {
        ...
        for (i, chunk) in iter.by_ref().enumerate() {
            ...
            chunk.copy_from_slice(&fptricks::batch_asymmetric_fma_cols_f64(
                tmp,
                mode.chunk::<4>(offset),
                sigma_lo.chunk::<4>(offset),
                sigma_hi.chunk::<4>(offset),
            ));
        }
        let rem = iter.into_remainder();
        let offset = limit & !3;
        for (i, val) in rem.iter_mut().enumerate() {
            ...
            let sigma = (sigma_lo.get(offset + i).to_bits() & zlt)
                | (sigma_hi.get(offset + i).to_bits() & zgeq);
        ...
        T: ParamSource<(f32, f32)>,
    {
        ...
        for (i, chunk) in iter.by_ref().enumerate() {
            ...
            for j in 0..8 {
                ...
                let xltz: u32 = ((x < 0.0) as u32).wrapping_neg();
                chunk[j << 1] = f32::from_bits(
                    (xltz & (m1[j] + x * sl1).to_bits()) | (!xltz & (m1[j] + x * sh1).to_bits()),
                );
                let x2 = r[j] * c[j];
                let (sl2, sh2) = sigs2[j];
                let x2ltz: u32 = ((x2 < 0.0) as u32).wrapping_neg();
    ...
    pub fn make_filled_std_normal_f32<const N: usize>(&mut self) -> [f32; N] {
        ...
        if i < rem.len() {
            rem[i].write(self.next_std_normal_f32());
        }
    ...
    pub fn make_filled_std_normal_f64<const N: usize>(&mut self) -> [f64; N] {
        ...
        if i < rem.len() {
            rem[i].write(self.next_std_normal_f64());
        }
        unsafe { buf.assume_init() }
    }
        ...
        S: ParamSource<f32, N>,
    {
        ...
        for (i, chunk) in iter.by_ref().enumerate() {
            ...
            chunk[0].write(res1[0]);
            chunk[1].write(res2[0]);
            chunk[2].write(res1[1]);
            chunk[3].write(res2[1]);
            chunk[4].write(res1[2]);
            chunk[5].write(res2[2]);
            chunk[6].write(res1[3]);
            chunk[7].write(res2[3]);
            chunk[8].write(res1[4]);
            chunk[9].write(res2[4]);
            chunk[10].write(res1[5]);
            chunk[11].write(res2[5]);
            chunk[12].write(res1[6]);
            chunk[13].write(res2[6]);
            chunk[14].write(res1[7]);
            chunk[15].write(res2[7]);
        }
        let rem = iter.into_remainder();
        let offset = N & !15;
        let mut i = 0;
    ...
    #[inline(always)]
    pub fn make_filled_normal_w_clamped_err_f32<M, S, L, const N: usize>(
        &mut self,
        mode: M,
        sigma: S,
        limit: L,
        ...
        L: ParamSource<f32, N>,
    {
        ...
        let mut iter = z.chunks_exact_mut(8);
        for (i, chunk) in iter.by_ref().enumerate() {
            let offset = i << 3;
            let lim = limit.chunk::<8>(offset);
            let mut arr = [0.0f32; 8];
    ...
    #[inline(always)]
    pub fn make_filled_normal_w_clamped_err_f64<M, S, L, const N: usize>(
        &mut self,
        mode: M,
        sigma: S,
        limit: L,
        ...
        L: ParamSource<f64, N>,
    {
        ...
        let mut iter = z.chunks_exact_mut(4);
        for (i, chunk) in iter.by_ref().enumerate() {
            let offset = i << 2;
            let lim = limit.chunk::<4>(offset);
            let mut arr = [0.0f64; 4];
    ...
    #[inline(always)]
    pub fn make_filled_ln_normal_f32<M, S, const N: usize>(
        &mut self,
        ln_mode: M,
        sigma_ln: S,
    ) -> [f32; N]
    ...
    #[inline(always)]
    pub fn make_filled_ln_normal_f64<M, S, const N: usize>(
        &mut self,
        ln_mode: M,
        sigma_ln: S,
    ) -> [f64; N]
    ...
    #[inline(always)]
    pub fn make_filled_log10_normal_f32<M, S, const N: usize>(
        &mut self,
        log_mode: M,
        sigma_log: S,
    ) -> [f32; N]
    ...
    #[inline(always)]
    pub fn make_filled_log10_normal_f64<M, S, const N: usize>(
        &mut self,
        log_mode: M,
        sigma_log: S,
    ) -> [f64; N]
    ...
    #[inline(always)]
    pub fn make_filled_split_normal_f32<M, SLO, SHI, const N: usize>(
        &mut self,
        mode: M,
        sigma_lo: SLO,
        sigma_hi: SHI,
    ...
    where
        M: ParamSource<f32, N>,
        SLO: ParamSource<f32, N>,
        SHI: ParamSource<f32, N>,
    {
        let mut buf = MaybeUninit::<[f32; N]>::uninit();
        let slice =
            unsafe { std::slice::from_raw_parts_mut(buf.as_mut_ptr() as *mut MaybeUninit<f32>, N) };
        ...
        for (i, chunk) in iter.by_ref().enumerate() {
            ...
            for j in 0..8 {
                ...
                let m = m1[j];
                let xltz: u32 = ((x < 0.0) as u32).wrapping_neg();
                chunk[j << 1].write(f32::from_bits(
                    (xltz & (m + x * sl1[j]).to_bits()) | (!xltz & (m + x * sh1[j]).to_bits()),
                ));
                let x2 = r[j] * c[j];
                let m2v = m2[j];
                let x2ltz: u32 = ((x2 < 0.0) as u32).wrapping_neg();
                chunk[(j << 1) + 1].write(f32::from_bits(
                    (x2ltz & (m2v + x2 * sl2[j]).to_bits())
                        | (!x2ltz & (m2v + x2 * sh2[j]).to_bits()),
                ));
            }
    ...
    #[inline(always)]
    pub fn make_filled_split_normal_f64<M, SLO, SHI, const N: usize>(
        &mut self,
        mode: M,
        sigma_lo: SLO,
        sigma_hi: SHI,
    ...
    where
        M: ParamSource<f64, N>,
        SLO: ParamSource<f64, N>,
        SHI: ParamSource<f64, N>,
    {
    ...
    #[inline(always)]
    pub fn make_filled_split_normal_stuple_f32<M, T, const N: usize>(
        &mut self,
        mode: M,
        table: T,
    ) -> [f32; N]
    ...
    #[inline(always)]
    pub fn make_filled_split_normal_stuple_f64<M, T, const N: usize>(
        &mut self,
        mode: M,
        table: T,
    ) -> [f64; N]
...
mod tests {
    ...
    fn test_split_normal_table_correctness() {
        ...
        if count_lo > 0 {
            let m = sum_lo / count_lo as f32;
            assert!(m < -0.5 && m > -1.1);
        }
        if count_hi > 0 {
            let m = sum_hi / count_hi as f32;
            assert!(m > 1.2 && m < 2.0);
        }
    }
    ...
    fn test_normal_nan_stability() {
        ...
        for _ in 0..10_000_000 {
            ...
            assert!(
                ...
                "NaN or Inf detected in scalar StdNormal"
            );
        }
        ...
        for _ in 0..10_000 {
            ...
            for &v in &buf {
                assert!(
                    ...
                    "NaN or Inf detected in batch StdNormal"
                );
            }
        }
    }
